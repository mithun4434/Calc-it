...
const StandardCalculator: React.FC<StandardCalculatorProps> = ({ onOpenSolver }) => {
    ...
    }, [display, isResultDisplayed, angleMode, history]);
    ...
    const baseButtonClasses = "aspect-square !text-2xl sm:!text-3xl transition-colors duration-200 active:scale-95";
    const glassButtonBase = "backdrop-blur-md border border-current/10";
    
    const specialFuncBtn = `bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 ${glassButtonBase} ${baseButtonClasses}`;
    const opBtn = `bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 ${glassButtonBase} ${baseButtonClasses}`;
    const numBtn = `bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/15 ${glassButtonBase} ${baseButtonClasses}`;
    const sciFuncBtn = `bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 ${glassButtonBase} ${baseButtonClasses} !text-xl sm:!text-2xl`;
    const angleToggleBtn = `${sciFuncBtn} !text-base sm:!text-lg`;

    return (
        <div className={`glass-panel w-full mx-auto p-2 sm:p-3 space-y-3 transition-all duration-300 ease-in-out rounded-3xl ${isScientific ? 'max-w-xl' : 'max-w-xs'}`}>
            <Button onClick={onOpenSolver} className="w-full bg-black dark:bg-white text-white dark:text-black hover:bg-gray-800 dark:hover:bg-gray-200 flex items-center justify-center gap-2 py-3 text-2xl">
                <SparklesIcon /> <span className="text-3xl">Problem Solver</span>
            </Button>
            ...
            <div className="relative bg-black/10 dark:bg-black/20 text-right rounded-3xl p-4 sm:p-6 overflow-x-auto break-all min-h-[8rem] flex items-end justify-end border border-current/10">
                <button 
                    ...
                    className="absolute top-2 right-2 p-2 opacity-60 hover:opacity-100 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-colors"
                ...
                <div className="flex gap-3">
                    ...
                    {isScientific && (
                         <div className="grid grid-cols-3 gap-3">
                            ...
                            <Button onClick={() => setAngleMode(p => p === 'deg' ? 'rad' : 'deg')} className={angleToggleBtn}>{angleMode.toUpperCase()}</Button>
                         </div>
                    )}
                    ...
                    <div className="flex-1 grid grid-cols-4 gap-3">
                        ...
                        <Button onClick={() => setIsScientific(!isScientific)} className={specialFuncBtn}>{isScientific ? 'std' : 'sci'}</Button>
